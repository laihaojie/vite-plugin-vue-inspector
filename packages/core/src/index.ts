...
export interface VitePluginInspectorOptions {
  ...
  /**
   * Enable long press to toggle inspector
   * @default true
   */
  longPress?: boolean
  ...
  return [
    {
      ...
      configureServer(server) {
        ...
        const { toggleComboKey, longPress } = normalizedOptions
        if (toggleComboKey && !longPress) {
          server.printUrls = () => {
          ...
          server.printUrls = () => {
            _printUrls()
            console.log(`  ${green('➜')}  ${bold('Vue Inspector')}: ${green(`Long press ${yellow('Control')} in the App to display the Inspector`)}\n`)
          }
        }
      },
