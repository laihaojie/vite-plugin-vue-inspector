...
export default {
  ...
  data() {
    return {
      ...
      longPress: inspectorOptions.longPress ?? false,
      longPressTime: inspectorOptions.longPressTime ?? 500,
      timerId: 0,
      position: {
        x: 0,
        y: 0,
  ...
  methods: {
    ...
    onKeyup(event) {
      if (event.key === undefined)
        return
      const isCombo = this.toggleCombo?.some(key => event.key.toLowerCase() === key)
      if (isCombo) {
        if (this.longPress) {
          this.toggleEnabled(false)
          if (this.timerId) {
...
<template>
  <div v-bind="{ [KEY_IGNORE]: 'true' }">
    <div
      v-if="containerVisible" ref="containerRef" class="vue-inspector-container" :style="containerPosition"
      :class="{ 'vue-inspector-container--disabled': !enabled }"
    >
      <!-- Logo -->
      <svg width="100" height="19" viewBox="0 0 62 12" fill="none" @click.prevent.stop="toggleEnabled(!enabled)">
        <path
        ...
        <path
          ...
          fill="url(#paint0_linear_2_49)" fill-opacity="0.94"
        />
        <path
      ...
      <!-- Banner  -->
      <a
        :style="bannerPosition" class="vue-inspector-banner vue-inspector-card"
        href="https://github.com/laihaojie/vite-plugin-vue-inspector" target="_blank"
      >
